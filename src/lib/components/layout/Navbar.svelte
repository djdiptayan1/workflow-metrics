<script lang="ts">
	...
	const dashboardHref = $derived(
		repoLabel ? `/dashboard?owner=${owner}&repo=${repo}` : '/dashboard'
	);
</script>

<header class="border-border bg-card flex h-14 flex-shrink-0 items-center gap-4 border-b px-6">
	...
	<div class="flex-1">
		...
		{:else if page.url.pathname === '/dashboard'}
			<span class="text-foreground text-sm font-medium">Repository Overview</span>
		{:else if page.url.pathname === '/settings'}
			<span class="text-foreground text-sm font-medium">Settings</span>
